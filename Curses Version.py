...
def main(stdscr):
    ...
    stdscr.clear() 
    colors = {
    'red': '█',        # Red block (using solid block character)
    'orange': '▒',     # Orange block (lighter block character)
    'yellow': '▓',     # Yellow block (shaded block character)
    'green': '█',      # Green block (using solid block character)
    'blue': '▓',       # Blue block (shaded block character)
    'purple': '▒',     # Purple block (lighter block character)
    'brown': '░',      # Brown block (lightest block character)
    'black': ' ',      # Empty space (blank)
    'white': '▒',      # White block (same as purple)
    'gray': '▒',       # Gray block (same as purple)
    'pink': '▓',       # Pink block (same as blue)
    'light_blue': '█'  # Light blue block (same as red)
}
